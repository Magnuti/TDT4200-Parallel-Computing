...
pixel im[9] = {
    ...
    (pixel){4, 4, 4}, (pixel){5, 50, 55}, (pixel){6, 6, 6}, // Note the difference in 5
...
{
    unsigned int sizeOfCol = width * height * filterDim * filterDim * numberOfChannels;
    unsigned char *col = malloc(sizeOfCol * sizeof(unsigned char));
    // pixel *im_from_col = malloc(9 * sizeof(pixel));

    im2col(im, col, width, height, filterDim);
